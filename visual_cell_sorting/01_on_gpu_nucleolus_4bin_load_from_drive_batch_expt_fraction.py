...
class LinuxService(rpyc.Service):
    class exposed_ImageAnalysis(object):
        ...
        def exposed_finalize(self):
            ...
            cell_dataframe = pd.DataFrame.from_dict(celldata,orient='index',columns=['ImageNumber', 'ImageDir', 'ObjectNumber', 'CellNumber', 'Nucleoli_Called', 'Nucleolar_Area', 'Nucleolar_Fraction', 'UV_Status'])
        ...
        def analyze_image(self, segmentation_image, intensity_image, image_dir):
            ...
            with warnings.catch_warnings():
                ...
                segmentation_image = self.normalize_image(segmentation_image, img_type='image', percentile=99.5)
                
                #normalize image for nucleolin
                intensity_image_norm = self.normalize_image(intensity_image, img_type='image', percentile=99.5)
                
                #perform segmentation
                masks, flows, styles, diams = model.eval(segmentation_image, diameter=56, normalize=False,
                ...
                for i,prop in enumerate(rps):
                    ...
                    #normalize crop
                    norm_crop = self.normalize_image(crop, img_type='crop', percentile=99.8)
                ...
                # Perform fastai predictions of nucleoli on gpu
                if n_cells > 0 and not random_activation:
                    #Classify crop as either having nucleolar signal, or not
                    
                    with learn_class.no_bar():
                        predictions = self.new_batch_pred_classification(batches, learn_class)
                    
                    batches = [batches[ii][preds==2] for ii, preds in enumerate(predictions)]
                    predictions = np.concatenate(predictions)
                    nuclear_areas_with_nucleoli = np.array(nuclear_areas)[predictions == 2]
                    n_cells_with_nucleoli = len(nuclear_areas_with_nucleoli)
                    num_new_batch = np.ceil(n_cells_with_nucleoli / batch_size)
                    concat_batch = torch.cat(batches)
                    batches = [concat_batch[i*batch_size:batch_size*(i+1)] for i in range(int(num_new_batch))]
                    ...
                    if n_cells_with_nucleoli > 0:
                        ...
                        nucleolar_ratios = np.divide(nucleolar_areas, nuclear_areas_with_nucleoli)
                        print(pd.Series(nucleolar_ratios).describe())
                ...
                for i in range(n_cells):
                    ...
                    if random_activation:
                        ...
                        if predictions[i] == 2:
                            nuc_area = nucleolar_areas[counter]
                            nuc_ratio = nucleolar_ratios[counter]
                            if nuc_ratio > 0:
                                if nuc_ratio >= nucleolar_ratio_cutoff_high + nucleolar_ratio_cutoff_offset:
                                    nucleolin_pred = 4
                                elif (nuc_ratio >= nucleolar_ratio_cutoff_mid + nucleolar_ratio_cutoff_offset) and (nuc_ratio < nucleolar_ratio_cutoff_high):
                                    nucleolin_pred = 3
                                elif (nuc_ratio >= nucleolar_ratio_cutoff_low) and (nuc_ratio < nucleolar_ratio_cutoff_mid):
                                    nucleolin_pred = 2
                                elif nuc_ratio <= nucleolar_ratio_cutoff_low - nucleolar_ratio_cutoff_offset:
                                    nucleolin_pred = 1
                            counter += 1
                    ...
                    celldata[object_number] = [
                        ...
                        i, 
                        predictions[i],
                        0 if predictions[i]!=2 else nuc_area,
                        0 if predictions[i]!=2 else nuc_ratio,
                        None
                    ]
                    object_number += 1
...
if __name__ == "__main__":
    ...
    img_save_dir = '/mnt/nas2/2023/Hyeon-Jin/07-16-23_F1-E15.25-nucleolar-fraction-plate5'
    img_move_dir = img_save_dir + '_images'
    mask_out_dir = img_save_dir + '_masks'
    prev_file = ''
    
    channel = 'NIR'                 #Experiment 1
    nucleolar_ratio_cutoff_low = 0.0771 # 0.0776, 0.0743, 0.0769
    nucleolar_ratio_cutoff_mid = 0.1013 # 0.0972, 0.0961, 0.1014
    nucleolar_ratio_cutoff_high = 0.1439 # 0.1299, 0.1369, 0.1462
    ...
    #Model
    classification_model_path = '/home/hyeonjin/Projects/fastai_models/nucleolin_filter_classifier/export.pkl'
    ...
    learn_class = load_learner(classification_model_path, cpu=False)
